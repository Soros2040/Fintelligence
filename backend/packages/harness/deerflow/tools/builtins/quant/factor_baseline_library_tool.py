...
@tool("factor_baseline_library", parse_docstring=True)
def factor_baseline_library_tool(
    runtime: ToolRuntime[ContextT, ThreadState],
    tool_call_id: Annotated[str, InjectedToolCallId],
    factor_set: str = "ALPHA20",
) -> Command:
    """查询基准因子库。列出指定因子集中所有因子的名称和Qlib表达式。

    可用的因子集：
    - ALPHA20: 20个精选因子（RD-Agent从Alpha158精选，不含VWAP依赖）
    - ALPHA158: 158个完整因子集（含1个VWAP依赖因子VWAP0）

    Args:
        factor_set: 因子集名称，默认ALPHA20
    """
    factors = FACTOR_SETS.get(factor_set)
    if factors is None:
        available = ", ".join(FACTOR_SETS.keys())
        return Command(
            update={"messages": [ToolMessage(f"未知因子集 '{factor_set}'。可用: {available}", tool_call_id=tool_call_id)]},
        )

    lines = [f"因子集 {factor_set}（共{len(factors)}个因子）："]
